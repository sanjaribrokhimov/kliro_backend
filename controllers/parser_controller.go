...
func (pc *ParserController) ParsePage(c *gin.Context) {
	...
	prompt := fmt.Sprintf(`Извлеки из текста условия кредита и верни результат в формате JSON со следующими полями:
- summa_max: максимальная сумма кредита (только число, без пробелов и валюты, если нет — null)
- srok_max: максимальный срок кредита в месяцах (только число, если нет — null)
- bank_name: название банка, возьми из url (например, если url \"https://www.ipoteka.uz/credits\", то \"ipoteka\", если не можешь — null)
- url: исходный url

Текст: "%s"
URL: "%s"

Ответ только в формате JSON, без пояснений. Если не можешь найти какое-то поле — ставь null.`, text, url)
